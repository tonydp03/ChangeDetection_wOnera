...
for f in folders:
    ...
    padded_raster = rnc.pad_raster(raster, img_size, stride)
    train_images = train_images + rnc.crop_raster(padded_raster, img_size, stride)    
...
for f in folders:
    ...
    padded_cm = rnc.pad_raster(cm, img_size, stride)
    train_labels = train_labels + rnc.crop_raster(padded_cm, img_size, stride)
...
# Train the model
history = model.fit(inputs, labels, batch_size=batch_size, epochs=epochs, validation_split=0.1, callbacks=[EarlyStopping(monitor='val_loss', patience=10, verbose=1, restore_best_weights=True)], shuffle=True, verbose=1)
...
print('Frozen model saved @ %s ' % save_dir)
